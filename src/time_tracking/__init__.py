...
def get_date_range(
    ...
    if day:
        ...
        stop_date = add_months(current_date, 1).replace(day=1) - datetime.timedelta(
            days=1
        )
    elif financial_year:
        if current_date.month < 7:
            start_date = current_date.replace(
                day=1, month=7, year=current_date.year - 1
            )
        else:
            start_date = current_date.replace(day=1, month=7)

        stop_date = current_date.replace(day=30, month=6, year=start_date.year + 1)

    return (start_date, stop_date)


def round_timedelta(
    td: datetime.timedelta, period: datetime.timedelta
) -> datetime.timedelta:
    """
...
class Entry:
    def __init__(
        ...
        *,
        ...
        break_delta: datetime.timedelta,
        t_raw_delta: datetime.timedelta,
        round_delta: datetime.timedelta,
...
class AggregatedTextEntry:
    def __init__(
        self, *, task: "Task", text: str, total_delta: datetime.timedelta
    ) -> None:
        self.task = task
        self.text = text
        self.total_delta = total_delta
...
class AggregatedDay:
    def __init__(self, *, date: datetime.date, total_delta: datetime.timedelta) -> None:
        self.date = date
        self.total_delta = total_delta
...
class AggregatedTask:
    def __init__(
        self, *, date: datetime.date, total_delta: datetime.timedelta, task: "Task"
    ) -> None:
        self.date = date
        self.total_delta = total_delta
        self.task = task
...
class EntryList:
        ...
        for date in self._dates:
            for entry in self._entries[date]:
                ...
                if employer not in days[date]:
                    days[date][employer] = {"total_delta": datetime.timedelta()}
        ...
        for date in self._dates:
            for entry in self._entries[date]:
                ...
                if project not in days[date]:
                    days[date][project] = {"total_delta": datetime.timedelta()}
    ...
    def task_report(self) -> Iterator[AggregatedTask]:
        days: Dict[
            datetime.date, Dict[Project, Dict[Task, Dict[str, datetime.timedelta]]]
        ] = {}
        ...
        for date in self._dates:
            for entry in self._entries[date]:
                ...
                if task not in days[date][project]:
                    days[date][project][task] = {"total_delta": datetime.timedelta()}
    ...
    def daily_report(self) -> Iterator[AggregatedDay]:
        ...
        for date in self._dates:
            for entry in self._entries[date]:
                if date not in days:
                    days[date] = {"total_delta": datetime.timedelta()}
                days[date]["total_delta"] += entry.total_delta

        dates = list(days.keys())
        dates.sort()
        for date in dates:
            yield AggregatedDay(date=date, total_delta=days[date]["total_delta"])
...
class Entries:
    def __init__(
        self, *, date_range: DateRange, location: Optional[str], files: List[str]
    ) -> None:
        ...
        for filename in self._files:
            ...
            for prev_entry, entry, next_entry in previous_and_next(root["entries"]):
                ...
                if project_name is None:
                    raise RuntimeError(
                        "%s: No project given for %s" % (filename, entry)
                    )
                ...
                if task is None:
                    raise RuntimeError(
                        "Unknown project '%s' task '%s'" % (project, task_name)
                    )
                ...
                if "start" not in entry:
                    if "stop" in entry and not next_entry_same_day:
                        continue
                    raise RuntimeError(
                        "%s: Cannot get start time for %s" % (filename, entry)
                    )
                ...
                if "stop" not in entry:
                    ...
                    if next_entry_same_day and "start" in next_entry:
                        ...
                        raise RuntimeError(
                            "%s: Cannot get stop time for %s" % (filename, entry)
                        )
                else:
                ...
                if entry_t_raw_delta < datetime.timedelta(0):
                    raise RuntimeError(
                        "%s: Entry %s  has negative delta" % (filename, entry_start_dt)
                    )
...
def report(
    date_range: DateRange, location: Optional[str], files: List[str], template_name: str
) -> None:
...
def main() -> None:
    ...
    group.add_argument(
        "-f", "--financial_year", dest="financial_year", action="store_true"
    )

    subparsers = parser.add_subparsers(dest="subparser")

    parser_a = subparsers.add_parser("report", help="Generate complete report.")
    parser_a.add_argument(
        "-t", "--template", dest="template_name", default="report.txt"
    )
